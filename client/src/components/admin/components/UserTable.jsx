...
const UserDetailDialog = ({
  ...
  const handleApproveTutor = async () => {
    try {
      const response = await approveTutorProfile(user.id);
  ...
  const handlePartialApproveTutor = async () => {
    try {
      const response = await partialApproveTutor(user.id);
      alert(response.message);
    } catch (error) {
        console.error("Error approving tutor:", error);
        alert(error.response.data.message);
  ...
  const handleRejectTutor = async () => {
    try {
      const response = await rejectTutorProfile(user.id);
      alert(response.message);
    } catch (error) {
      console.error("Error approving tutor:", error);
      alert(error.response.data.message);
                  ...
                  <AccordionSummary expandIcon={<ExpandMore />}>
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                      ...
                      <FormControlLabel
                        control={
                          <Switch
                            checked={user.interviewSlots.map(slot => slot.is_interview).includes(true)}
