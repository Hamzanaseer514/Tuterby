...
const SubjectLevelCard = ({ level, onExpand, isExpanded, isHomePage }) => {
  ...
  const { name, icon: LevelIcon, description, coreSample, subjects, path, isPremiumService } = level;
  const expandAriaLabel = isExpanded ? `Collapse full list for ${name}` : `Expand full ${isPremiumService ? "feature" : "subject"} list for ${name}`;
  const learnMoreAriaLabel = `Learn more about ${name} ${isPremiumService ? "programme" : "tutoring"}`;


  const displayedSubjects = [];
  if (coreSample && subjects) {
    coreSample.forEach(coreName => {
      const coreSubject = subjects.find(s => s.name.toLowerCase().includes(coreName.toLowerCase()));
  ...
  const headerIcon = isValidLucideIcon(LevelIcon) ? LevelIcon : (name === "Undergraduate" ? BookOpen : ListChecks);
      ...
      <Card className="h-full flex flex-col bg-card/80 dark:bg-card/90 backdrop-blur-sm border-2 border-primary/10 hover:border-primary/30 transition-all duration-300 shadow-lg hover:shadow-2xl rounded-2xl overflow-hidden">
        ...
        <CardContent className="p-6 flex-grow">
          <p className="text-sm font-semibold text-foreground mb-3">
            {isPremiumService ? "Key Features Include:" : "Popular Subjects/Modules:"}
          </p>
          <ul className="space-y-2.5">
            {displayedSubjects.map((subject, index) => {
              const SubjectIcon = subject.icon;
              return (
                <li key={index} className="flex items-center text-sm text-foreground/90 dark:text-slate-300">
                   {isValidLucideIcon(SubjectIcon) && <SubjectIcon className={`w-4 h-4 mr-2.5 ${isPremiumService ? 'text-secondary' : 'text-primary'}`} />}
        ...
        <CardFooter className="p-6 bg-muted/30 dark:bg-slate-800/30 mt-auto">
          <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            >
              {isExpanded ? 'Show Less' : (isPremiumService ? 'Explore All Features' : 'View All Subjects')}
              <ChevronRight className={`ml-2 h-4 w-4 transition-transform duration-300 ${isExpanded ? 'rotate-90' : ''} group-hover:translate-x-0.5`} />
            </Button>
            {!isHomePage && level.path && (
              <Button asChild variant="ghost" className="w-full sm:w-auto text-primary hover:bg-primary/10 hover:text-primary">
