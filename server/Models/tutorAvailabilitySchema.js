...
// Schema for one-time availability slots
...
const tutorAvailabilitySchema = new mongoose.Schema({
  ...
  recurring_availability: [recurringAvailabilitySchema],
  // One-time availability slots
  one_time_availability: [oneTimeAvailabilitySchema],
  // Blackout dates
...
tutorAvailabilitySchema.index({ 'one_time_availability.date': 1 });
...
tutorAvailabilitySchema.methods.isAvailable = function(date, durationMinutes = 60) {
  ...
  // If we reach here, the time is within general availability hours
  // Now check if there are any specific recurring or one-time slots that might override this
  
  // Check recurring availability
  const hasRecurringSlot = this.recurring_availability.some(slot => 
    slot.is_active &&
  ...
  const hasOneTimeSlot = this.one_time_availability.some(slot => 
    slot.is_active &&
    slot.date.toDateString() === checkDate.toDateString() &&
    timeString >= slot.start_time &&
    timeString <= slot.end_time
  );
